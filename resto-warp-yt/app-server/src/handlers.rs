...
pub async fn create_table_handler(
    ...
    match Table::get_existing_table_id(&conn, &data) {
        Ok(Some(table_id)) => Ok(warp::reply::with_status(
            warp::reply::json(&json!({"id": table_id})),
            warp::http::StatusCode::CREATED,
        )),
        Ok(None) => match Table::create(&conn, &data) {
...
pub async fn create_menu_handler(
    ...
    match Menu::get_existing_menu_id(&conn, &data) {
        Ok(Some(menu_id)) => Ok(warp::reply::with_status(
            warp::reply::json(&json!({"id":menu_id})),
            warp::http::StatusCode::CREATED,
        )),
        Ok(None) => match Menu::create(&conn, &data) {
...
pub async fn create_order_handler(
    ...
    if menu_ids.len() == 0 {
        return Ok(warp::reply::with_status(
            warp::reply::json(&json!({"error":"no menu ids"})),
    ...
    match OrderResponse::get_existing_order_id(&conn, table_id) {
        Ok(Some(order_id)) => {
            for menu_id in menu_ids {
                ...
                match OrderItem::get_existing_order_item_id(&conn, order_id, menu_id) {
                    ...
                    Ok(None) => match OrderItem::create(&conn, order_id, menu_id, cooking_time) {
                        Ok(_) => {
                            continue;
                        }
                        Err(_) => {
                    ...
                    Err(_err) => {
                        return Ok(warp::reply::with_status(
                            warp::reply::json(
                                &json!({"error": "error creating for existing order Item"}),
                            ),
                            warp::http::StatusCode::INTERNAL_SERVER_ERROR,
                        ))
                    }
        ...
        Ok(None) => match OrderResponse::create(&conn, table_id) {
            Ok(last_inserted_id) => {
                for menu_id in menu_ids {
                    let cooking_time = rand::thread_rng().gen_range(5..=15);
                ...
                Ok(warp::reply::with_status(
                    warp::reply::json(
                        &json!({"id": last_inserted_id, "success": "Order and all order Item created successfully"}),
                    ),
                    warp::http::StatusCode::CREATED,
                ))
            }
...
mod tests {
    ...
    fn setup_static_data(conn: &Connection) {
        ...
        for value in values_to_insert {
            conn.execute("INSERT INTO tables (code) VALUES (?1)", &[value])
                .expect("Insertion Failed");
        }
        let values_to_insert = vec!["M-01", "M-02", "M-03", "M-04", "M-05"];

        for value in values_to_insert {
            conn.execute("INSERT INTO menus (name) VALUES (?1)", &[value])
                .expect("Insertion Failed");
        }
    }
