...
async fn create_tables() -> Vec<i64> {
    ...
    for index in 0..5 {
        ...
        let response: Value = client
            .post("http://localhost:8888/tables/create")
...
async fn create_menus() -> Vec<i64> {
    ...
    for index in 0..5 {
        ...
        let response: Value = client
            .post("http://localhost:8888/menus/create")
    ...
    let handles: Vec<_> = (0..10)
        .map(|_| {
            ...
            tokio::spawn(async move {
                // 1. Create Order
                let response = client
                    .post("http://localhost:8888/orders/create")
                ...
                let response = client
                    .get(&format!("http://localhost:8888/tables/{}/items", table_id))
                    .send()
                    .await
                ...
                if let Some(menu_id) = menu_subarray.first() {
                    let response = client
                        .get(&format!(
                            "http://localhost:8888/tables/{}/items/{}",
                            table_id, *menu_id
                        ))
                        .send()
                        .await
                    ...
                    println!(
                        "Menu {} from table {} is: Menu: {:?}, Cooking Time: {:?}, Quantity: {:?}",
                        menu_id,
                        table_id,
                ...
                if let Some(menu_id) = menu_subarray.first() {
                    let response = client
                        .delete(&format!(
                            "http://localhost:8888/orders/{}/items/{}",
                            table_id, *menu_id
                        ))
                        .send()
                        .await
