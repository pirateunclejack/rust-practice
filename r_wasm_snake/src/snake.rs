...
impl Snake {
    pub fn new(width: u32, height: u32) -> Snake {
        let head_x: u32 =
            stdweb::unstable::TryInto::try_into(js! {return Math.floor(Math.random() * @{width})})
                .unwrap();

        let head_y: u32 =
            stdweb::unstable::TryInto::try_into(js! {return Math.floor(Math.random() * @{height})})
                .unwrap();
    ...
    pub fn update(&mut self) {
        ...
        if self.head == self.food {
            ...
            while food == self.head || self.tail.contains(&food) {
                let food_x: u32 = stdweb::unstable::TryInto::try_into(
                    js! {return Math.floor(Math.random() * @{self.width})},
                )
                .unwrap();

                let food_y: u32 = stdweb::unstable::TryInto::try_into(
