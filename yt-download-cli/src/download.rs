...
async fn downloadfromstream(prefix: &str, url: &str, path: &str) -> Result<()> {
    ...
    let filename1 = response
        ...
        .to_str()
        .ok();
    let filename2 = filename1.unwrap().strip_prefix("attachment; filenam=\"");
    let filename3 = filename2.unwrap().strip_prefix("\"").unwrap();
...
async fn getstream(prefix: &str, url: &str, body: HashMap<&str, &str>, path: &str) {
    ...
    let formatted_response = response
        .expect("method not find in `Result<Response, Error>`")
        .text()
        .await
        .unwrap();
